...
output_folder + "/1901_to_2010/" + 'baseflow_monthTot_output.nc',
output_folder + "/1901_to_2010/" + 'directRunoff_monthTot_output.nc',
output_folder + "/1901_to_2010/" + 'interflowTotal_monthTot_output.nc'
]
...
for i_year in range(0, len(year_int)-1):
    cmd += 'python nc_basin_merge.py %s %s/%i_to_%i/ %i %i-01-31 %i-12-31 selected_monthly_runoff_components & ' %(input_folder[i_year]         , output_folder, year_int[i_year]         , year_int[i_year+1]-1, num_of_cores, year_int[i_year]         , year_int[i_year+1]-1)
cmd     += 'python nc_basin_merge.py %s %s/%i_to_%i/ %i %i-01-31 %i-12-31 selected_monthly_runoff_components & ' %(input_folder[len(year_int)-1], output_folder, year_int[len(year_int)-1], last_year,            num_of_cores, year_int[len(year_int)-1], last_year           )
cmd     += 'wait'
print cmd
os.system(cmd)
